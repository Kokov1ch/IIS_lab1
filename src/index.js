...
function createDropDown() {
    const groupObj = d3.group(bananas, d => d.Country);
...
function applyFilter() {
    ...
    const sizeOrder = d3.select("#sortBySize").property("value");
    const weightOrder = d3.select("#sortByWeight").property("value");
    sortTable(sizeOrder, weightOrder);
...
function sortTable(sizeOrder, weightOrder) {
    const table = d3.select("div.table").select("table");
    let rows = table.selectAll("tr").data();

    let sizeSortFunction = getSortFunction("Size", sizeOrder);
    let weightSortFunction = getSortFunction("Weight", weightOrder);
...
while (index < bananas.length) {
    const chunk = bananas.slice(index, index + 100);
    const data = {
        t: t_index,
        data: chunk.map((banana, chunkIndex) => ({x: chunkIndex, y: Math.abs(parseFloat(banana.Weight))}))
    };
    dataTemp.push(data);
    index += 100;
    t_index++;
